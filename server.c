...
int main() {
    ...
    while(1) {
        if ((new_socket = accept(conn_sckt, (struct sockaddr *)&address, (socklen_t*)&addrlen))<0) {
            perror("accept_main");
        ...
        if(child_pid == 0) {
            ...
            crequest(new_socket); // Handle the client's request
            exit(0);
        } 
        else {
            close(new_socket); // Close the connected socket in the parent process
        }
    }
